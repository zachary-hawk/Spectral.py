...
class Spectral:
    ...
    def plot_bs(self,
        ...
        ax.tick_params(axis='both', which='minor', labelsize=fontsize*0.8,length=6, right=True, top=False, bottom=False,left=True,width=1.2,direction="in")
    ...
    def plot_dos(self,
        ...
        if swap_axes:
            ...
            ax.tick_params(axis='both', which='major', labelsize=fontsize*0.8,length=12,width=1.2,direction="in")
            ax.tick_params(axis='both', which='minor', labelsize=fontsize*0.8,length=6, right=True, top=False, bottom=False,left=True,width=1.2,direction="in")
            ...
            ax.tick_params(axis='both', which='major', labelsize=fontsize*0.8,length=12,width=1.2,direction="in")
            ax.tick_params(axis='both', which='minor', labelsize=fontsize*0.8,length=6, right=True, top=False, bottom=True,left=True,width=1.2,direction="in")
        ...
        if not pdos:
            ...
            if recalculate_dos:
                if broadening=='gaussian':
                    ...
                    for nb in range(self.nbands):            
                        for ns in range(self.nspins):
                            all_dos[nb,:,ns,:]=self._gaussian(new_bs[nb,:,ns,:],E,width)*new_kpt_w[:,ns]
                    ...
                    for nb in range(self.nbands):            
                        for ns in range(self.nspins):
                            all_dos[nb,:,ns,:]=self._adaptive(new_bs[nb,:,ns,:],E,new_weights[nb,:,ns,:])*new_kpt_w[:,ns]
    ...
    def plot_single_pdos(self,
                         ...
                         width=0.05,
                         loc='upper right'):
        ''' This is a different pdos plot specifically for plotting the individual orbital components of a single ion.'''
        ...
        # Set the size based on these params.
        n_orbs = len(self.pdos_filter(species,orbital,ion))
        pdos_dos=np.zeros((n_orbs,len(E),self.nspins))
        ...
        # Store the broadening
        if self.broadening!=broadening:
            if not recalculate_dos:
                recalculate_dos = True
        self.broadening = broadening
        ...
        # Here are all of the labels coming from castep
        ...
        if recalculate_dos:
            ...
            if broadening=='gaussian':
                ...
                for iorb in range(n_orbs):                    
                    pdos_factor = self.pdos_filter(species,orbital,ion)[iorb]
                    pdos_factor= np.repeat(pdos_factor[:,:,:,np.newaxis],len(E),axis=3)
                    # Multiply in the factor
                    temp_dos = all_dos *pdos_factor 
                    ...
                    temp_dos= np.sum(temp_dos,axis=0)
                    temp_dos= np.sum(temp_dos,axis=0)
                    pdos_dos[iorb,:,:]= np.swapaxes(temp_dos,0,1)
        ...
        if colors is not None:
            try:
                assert len(colors)==n_orbs
                color=colors
                custom_cycler = (cycler.cycler(color=color))
                ax.set_prop_cycle(custom_cycler)
            
            except:
                warnings.warn("Warning: colors does not match number of suborbitals")

                color = plt.cm.bwr(np.linspace(0,1,n_orbs))
                for i in range(len(color)):
                    if np.all(np.round(color[i,0:3])==1):
                        color[i,0:3] = [0.5,0.5,0.5]
                custom_cycler = (cycler.cycler(color=color))
        ...
        if not swap_axes:
            ...
            for iorb in range(n_orbs):
                color=next(ax._get_lines.prop_cycler)['color']
                for s in spin_index:                                
                    ax.plot(E,pdos_dos[iorb,:,s],linestyle=linestyle,linewidth=linewidth,label=self.atoms[species]+"("+orbs[iorb]+')',color=color,zorder=iorb)
                    if shade:
                        ax.fill_between(E,pdos_dos[iorb,:,s],alpha=alpha,color=color,zorder=iorb)
            ...
            for iorb in range(n_orbs):
                color=next(ax._get_lines.prop_cycler)['color']        
                for s in spin_index:                                
                    ax.plot(pdos_dos[iorb,:,s],E,linestyle=linestyle,linewidth=linewidth,label=self.atoms[species]+"("+orbs[iorb]+')',color=color,zorder=iorb)
                    if shade:
                        ax.fill_betweenx(E,pdos_dos[iorb,:,s],alpha=alpha,color=color,zorder=iorb)
        ...
        ax.legend(handles, labels, loc=loc,fontsize=fontsize*0.8,fancybox=True,frameon=False,handlelength=1.5,handletextpad=0.2)
